...
wangzz（以下简称"我们"）高度重视用户隐私保护。本政策说明我们在您使用 AI换脸（以下简称"本应用"）时如何收集、使用、存储、共享您的个人信息，以及您对这些信息的控制方式。请您在使用本应用前仔细阅读本政策。
...
- 设备型号与 iOS 系统版本（仅用于崩溃诊断与功能适配，不上传您的人脸图像）
- 订阅凭证：仅本地 Keychain 存储，用于恢复购买
- 应用使用统计：模板使用频次、错误率（仅本地记录，不含个人身份信息）
...
- **腾讯云 AI**（人像变换、动漫化、美颜）：为实现变老、变性、动漫化、智能美颜等核心功能所必需。腾讯云仅在内存中处理您主动上传的人脸图像，**不持久化存储、不用于模型训练**。服务条款：https://cloud.tencent.com/document/product/301
- **Apple StoreKit**（内购与订阅）：用于处理订阅凭证与恢复购买。隐私协议：https://www.apple.com/legal/privacy/

我们已与上述第三方建立数据处理协议，要求其按照不低于本政策的标准处理您的信息。
...
- 撤回您先前授予的相册权限（iOS 设置 → 隐私与安全性 → AI换脸）
...
wangzz ("we", "us", "our") values your privacy. This Privacy Policy describes how we collect, use, store, and share your personal information when you use AI换脸 (the "App"), and the choices you have regarding that information. Please read this policy carefully before using the App.
...
- Device model and iOS version (only for crash diagnostics and feature compatibility; not used to upload your face images)
- Subscription credentials: stored only in local Keychain for purchase restoration
- App usage statistics: template usage frequency, error rates (recorded locally only, no personal identity information)
...
- To provide the App's core functions (face transformation, template swap, beautification, anime-style)
...
- **Tencent Cloud AI** (face transformation, anime-style, beautification): required to deliver the core functions of age change, gender change, anime-style, and beautification. Tencent Cloud processes your uploaded face images in memory only; it does not persistently store them and does not use them for model training. Terms: https://cloud.tencent.com/document/product/301
...
- Withdraw previously granted photo library permissions (iOS Settings → Privacy & Security → AI换脸)
...
- Display an explicit **"AI 合成 · AI换脸"** watermark
- Embed **C2PA** metadata
